...
export const SummaryCards: React.FC<SummaryCardsProps> = ({ yearly, comparison }) => {
  ...
  // 确保数字在一行完整显示，根据长度动态调整字体大小
  // 使用更保守的字体大小，确保即使是最长的数字也能在一行显示
  const getFontSizeClass = (len: number) => {
    if (len > 18) return "text-xs sm:text-sm lg:text-base"; // 超长数字，使用更小字体
    if (len > 15) return "text-sm sm:text-base lg:text-lg"; // 极长数字
    if (len > 12) return "text-base sm:text-lg lg:text-xl"; // 长数字
    return "text-lg sm:text-xl lg:text-2xl"; // 标准
  };
  ...
  return (
    <div className="space-y-6 mb-6">
      ...
      {showRecommendation && (
        <div className="glass-card bg-gradient-to-r from-yellow-50/80 to-orange-50/80 border border-yellow-200/50 rounded-xl p-4 flex items-start">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {/* Total Net Income */}
        <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-xl shadow-xl p-6 text-white transform transition hover:scale-105 sm:col-span-2 lg:col-span-1 relative overflow-hidden backdrop-blur-sm border border-white/20">
          <div className="relative z-10 flex justify-between items-start">
            <div className="flex-1 min-w-0 overflow-hidden">
              <p className="text-emerald-100 text-sm font-medium">全年到手总收入</p>
              <h3 className={`${netIncomeClass} font-bold mt-1 tracking-tight transition-all duration-300 whitespace-nowrap`} title={netIncomeStr}>
                {netIncomeStr}
              </h3>
            </div>
          </div>
        ...
        {/* Total Tax */}
        <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-xl p-6 text-white transform transition hover:scale-105 relative overflow-hidden backdrop-blur-sm border border-white/20">
           <div className="relative z-10 flex justify-between items-start">
            <div className="flex-1 min-w-0 overflow-hidden">
              <p className="text-red-100 text-sm font-medium">全年缴纳个税</p>
              <h3 className={`${taxClass} font-bold mt-1 tracking-tight transition-all duration-300 whitespace-nowrap`} title={totalTaxStr}>
                {totalTaxStr}
              </h3>
            </div>
          </div>
        ...
        {/* Social Security */}
        <div className="bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl shadow-xl p-6 text-white transform transition hover:scale-105 relative overflow-hidden backdrop-blur-sm border border-white/20">
           <div className="relative z-10 flex justify-between items-start">
            <div className="flex-1 min-w-0 overflow-hidden">
              <p className="text-amber-100 text-sm font-medium">个人五险一金</p>
              <h3 className={`${socialClass} font-bold mt-1 tracking-tight transition-all duration-300 whitespace-nowrap`} title={totalSocialStr}>
                {totalSocialStr}
              </h3>
            </div>
          </div>
